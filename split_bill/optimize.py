...
def optimize(balance):
    ...
    for l in range(n_start):
        ...
        for k in range(n_iter):
            prob.solve()
            W.value = 1 / (delta + np.abs(x.value)) + (np.abs(np.random.randn(n*n)+1e-5))
    ...
    return {p: {p2: x_mat[i,j] for j, p2 in enumerate(participants)} for i,p in enumerate(participants)}
